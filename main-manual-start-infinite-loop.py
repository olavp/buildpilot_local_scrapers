...
while True:
    ...
    # for dev/debug: skip sleep on entry
    if not first_loop:
        print('Sleep 30 secs')
        time.sleep(30)
        print('awake')
    ...
    # skip if nighttime
    print("Test 1: is it daytime now?")
    hour_now = datetime.now(ZoneInfo("Europe/Oslo")).hour
    print(f"hour_now: {hour_now}")
    if hour_now < 8 or hour_now > 18:
        print(' --> failed test 1: Do not run at nighttime')
        continue
    print(' --> passed test 1')
    print()
    ...
    if next_in_queue["status"] == "no-queue":
        ...
        print(f' --> failed test 3: next_in_queue["status"] == "no-queue" --> do not run')
        continue

    print(' --> passed test 3')
    print()
    print('passed all test -> do run')
    print()
    ...
    if is_logged_in is False:
        ...
        datalogalert(
            ...
            alert_name=None,
            counters=None,
            comment="Cannot find button with logged in",
        )
        sys.exit()
